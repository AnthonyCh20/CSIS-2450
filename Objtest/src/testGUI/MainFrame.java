...
public class MainFrame extends JFrame {
	...
	private void displayCard(JButton searchBtn) {
		searchBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				dispose();
				TestCall tc = new TestCall("https://api.pokemontcg.io/v2/cards?q=name:Bulbasaur&page=1&pageSize=1");
				CardDisplay cd = new CardDisplay(tc.getCardImg(), tc.getPokemonName(),tc.getId(),tc.getSuperType(),tc.getEvolution(),tc.getSubType(),tc.getWeakness(),tc.getHp());
				cd.setVisible(true);
			}
		});
	}
	...
	/**
	 * @param myCollectionBtn
	 * Closes main page and opens collection page
	 */
